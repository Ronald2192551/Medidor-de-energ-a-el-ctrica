...
const float VOLTAGE_DIVIDER_RATIO = 169.7 / 1.0;
...
void publishHistory() {
  ...
  for (int i = 0; i < BUFFER_SIZE; i++) {
    ...
    data += String(i + 1) + ". V=" + String(voltage_history[index], 1) + "V C=" + String(current_history[index], 3) + "A\n";
  }

  if (client.publish(mqtt_topic_data, data.c_str())) {
    Serial.println("📤 Datos enviados a EMQX");
    Serial.print("   Tamaño: ");
...
void loop() {
  ...
  if (dataReady) {
    ...
    digitalWrite(CS_PIN, LOW);
    voltage_raw = read_ADC_channel(0);
    current_raw = read_ADC_channel(1);
    digitalWrite(CS_PIN, HIGH);
